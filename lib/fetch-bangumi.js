...
function rewriteImageUrl(url, proxy) {
  if (!proxy || !url) return url;
  // https://lain.bgm.tv/pic/cover/l/... → {proxy}/img/pic/cover/l/...
  return url.replace(`https://${IMAGE_HOST}`, proxy.replace(/\/+$/, '') + '/img');
}

function normalizeCollection(item, proxy) {
  ...
  const rawCover = subject.images?.large || subject.images?.common || subject.images?.medium || '';

  return {
    id: subject.id || item?.subject_id,
    title: subject.name_cn || subject.name || '未知条目',
    originalTitle: subject.name || '',
    cover: rewriteImageUrl(rawCover, proxy),
...
async function fetchCollectionsByStatus(client, { username, type, label, pageSize, showProgress, proxy }) {
  const items = [];
  let offset = 0;
  let total = null;
  ...
  while (total === null || offset < total) {
    ...
    items.push(...pageItems.map((item) => normalizeCollection(item, proxy)));
    offset += pageSize;
